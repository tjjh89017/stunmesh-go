...
with `GOOS=linux` and `MOBILE_TAGS` (= `builtin_all mobile`, so the real
built-in plugin implementations are linted rather than their stubs); CI's
mobile lint step passes the same two tags.

### Dependency Management
```bash
go mod tidy                      # Clean up dependencies
go generate ./internal/entity    # Regenerate mocks (requires mockgen)
go generate ./app                # Regenerate wire dependency injection (directive lives in app/wire_gen.go)
```
...
The `-s` flag adds a `Signed-off-by` line to the commit message, which is required for all commits.
...
- Commits should reflect the human author's intent and responsibility
- AI tools are development aids, not co-authors

**Example:**
```bash
# Good
git commit -s -m "feat: add periodic GC and memory cleanup goroutine"

# Bad - don't do this
git commit -m "feat: add GC goroutine

Generated by Claude"
```

## Release and Downstream Update Flow

Cutting a stunmesh-go release:
- Create an annotated, unsigned tag `vX.Y.Z` and push it. `release.yml` builds binaries, contrib plugin archives, the Android AAR and the Docker image, and creates an empty draft release.
- Write the release notes by hand in prose, then un-draft with `gh release edit vX.Y.Z --draft=false --latest`.

Downstream repos, in this order (each item: what to change, how to release):

1. **stunmesh-provisioner**. Dependabot (gomod, daily) opens a PR bumping `github.com/tjjh89017/stunmesh-go` in `go.mod`. To do it by hand: `go get github.com/tjjh89017/stunmesh-go@vX.Y.Z && go mod tidy`. Merge, then push a `v*` tag if a provisioner release is wanted. stunmesh-agent embeds the `app` package, so a provisioner release usually follows a stunmesh-go release.
2. **stunmesh-openwrt**. Edit `net/stunmesh-go/Makefile`: `PKG_VERSION`, reset `PKG_RELEASE:=1`, refresh `PKG_HASH` with `curl -sL https://codeload.github.com/tjjh89017/stunmesh-go/tar.gz/vX.Y.Z | sha256sum`. If provisioner released, do the same in `net/stunmesh-provisioner/Makefile`. Push to main; `build.yml` republishes the apk feed. There are no tags.
3. **stunmesh-openwrt-firmware**. Nothing to edit; it installs whatever the stunmesh-openwrt feed serves at build time. Trigger `firmware.yml` with `workflow_dispatch` after the feed build finishes (a push to main also builds, and the weekly cron picks it up otherwise). Every run publishes a `firmware-<build_id>` release.
4. **stunmesh-opnsense**. Edit `Makefile`: `PLUGIN_VERSION`, reset `PLUGIN_REVISION=1`. Refresh `distinfo` from `gh release view vX.Y.Z --repo tjjh89017/stunmesh-go --json assets --jq '.assets[] | "\(.name) \(.digest)"'`. Push to main; `build.yml` builds the pkg and republishes the feed. There are no tags.
5. **stunmesh-android**. Edit `gradle.properties`: `stunmeshCoreVersion=vX.Y.Z`. Push a `v*` tag to release.
6. **stunmesh-docs**. For a new minor version run `pnpm docusaurus docs:version X.Y`, which snapshots `docs/` into `versioned_docs/version-X.Y` and prepends `versions.json`. Check `docs/guides/openwrt.md` for the OpenWrt series in the feed URL. Push to main deploys.

Each downstream repo has its own copy of its step in its README or CLAUDE.md; keep both in sync.

## Core Architecture

`docs/architecture.md` records the codebase-wide structural patterns
(narrow consumer-owned interfaces, why proxy wiring lives in `app`, the
plugin registry's `init()` self-registration, and so on) — consult and
update it when making structural changes.

### Composition Root: the `app` Package
`app` is the composition root, and the only place the daemon and its
dependencies are wired together — both the CLI (`main.go`) and any external
...
The application uses Google Wire for dependency injection. The setup is in `app/wire.go`:
- `app/wire.go`: Defines dependency graph (build tag: `wireinject`)
- `app/wire_gen.go`: Generated dependency injection code (do not edit manually)
- Run `go generate ./app` after changing dependencies (the generate directive lives in `app/wire_gen.go`)
...
2. **PublishController** (`internal/ctrl/publish.go`):
   ...
   - Uses the shared `ctrl.DiscoverEndpoints` (`internal/ctrl/discover.go`) for protocol-aware STUN discovery — also used by `mobile/`, so policy changes there affect both platforms. Dualstack soft-fails per family (warn and continue; error only when both families come back empty); single-family modes hard-fail
...
3. **EstablishController** (`internal/ctrl/establish.go`):
   ...
   - Decrypts endpoint JSON and selects the endpoint via the shared `ctrl.SelectEndpoint` (`internal/ctrl/endpoint_select.go`, also used by `mobile/`) based on peer protocol:
...
Examples of interface bindings in `app/wire.go` (see the file for the full set — there are around a dozen, including the daemon runner and ctrl publisher/establisher bindings):
```go
...
1. Update `app/wire.go` with new bindings
2. Run `go generate ./app` to regenerate `wire_gen.go` (the directive lives in app/wire_gen.go)
