...
4. **stunmesh-opnsense**. Edit `Makefile`: `PLUGIN_VERSION`, reset `PLUGIN_REVISION=1`. Refresh `distinfo` from `gh release view vX.Y.Z --repo tjjh89017/stunmesh-go --json assets --jq '.assets[] | "\(.name) \(.digest)"'`. Push a `v*` tag to release.
