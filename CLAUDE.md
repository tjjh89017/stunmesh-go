...
3. **stunmesh-openwrt-firmware**. Nothing to edit; it installs whatever the stunmesh-openwrt feed serves at build time. Rebuild now with `workflow_dispatch` on `firmware.yml`, otherwise the weekly cron picks it up. Every run publishes a `firmware-<build_id>` release.
